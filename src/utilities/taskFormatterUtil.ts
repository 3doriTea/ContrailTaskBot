...
export const genTasksData = (json: GoogleSpreadSheetJson): Array<TaskData> => {
  ...
  const toDate = (dateStr: string) => {
    return dfns.parse(dateStr, dateFormat, new Date());
  };
  const toRate = (rateStr: string) => {
    return Number.parseInt(rateStr.substring(0, rateStr.length - 1)) / 100.0;
  };

  return json.values
    .splice(config.begin.wBS.row)
    .map((line): TaskData | undefined => {
      ...
      return {
        ...
        taskCompletionRate: toRate(
          line[column(config.begin.taskCompletionRate)]
        ),
        taskDaysToStart: Number.parseInt(
